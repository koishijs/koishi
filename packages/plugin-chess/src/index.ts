  ...
  ctx.command('chess [position]', '棋类游戏')
    ...
    .action(async ({ meta, options }, position) => {
      if (!states[meta.groupId]) {
        ...
        return state.draw(meta, `${meta.$username} 发起了游戏！`)
      }

      if (options.stop) {
        delete states[meta.groupId]
        meta.$group.chess = null
      ...
      if (options.show) return state.draw(meta)
      ...
      if (options.skip) {
        ...
        meta.$group.chess = state.serialize()
        return meta.$send(`${meta.$username} 选择跳过其回合，下一手轮到 [CQ:at,qq=${state.next}]。`)
      }
      ...
      if (options.repent) {
        ...
        state.next = last
        meta.$group.chess = state.serialize()
        return state.draw(meta, `${meta.$username} 进行了悔棋。`)
      }
      ...
      if (!state.p2) {
        ...
        message = `${meta.$username} 加入了游戏并落子于 ${position.toUpperCase()}，`
      } else {
        message = `${meta.$username} 落子于 ${position.toUpperCase()}，`
      }
