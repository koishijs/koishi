...
namespace QueryOperators {
  ...
  export const comparison = function Comparison(app: App) {
    before(async () => {
      ...
      await app.database.create('foo', { text: 'awesome foo', date: new Date('2000-01-01') })
      await app.database.create('foo', { text: 'awesome bar' })
      await app.database.create('foo', { text: 'awesome baz' })
    })

    it('basic support', async () => {
      await expect(app.database.get('foo', {
        id: { $eq: 2 },
  ...
  export const membership = function Membership(app: App) {
    before(async () => {
      await app.database.remove('foo', {})
      await app.database.create('foo', { value: 3 })
      await app.database.create('foo', { value: 4 })
      await app.database.create('foo', { value: 7 })
    })
  ...
  export const regexp = function RegularExpression(app: App, options: RegExpOptions = {}) {
    ...
    before(async () => {
      await app.database.remove('foo', {})
      await app.database.create('foo', { text: 'awesome foo' })
      await app.database.create('foo', { text: 'awesome bar' })
      await app.database.create('foo', { text: 'awesome foo bar' })
    })
  ...
  export const list = function List(app: App, options: ListOptions = {}) {
    ...
    before(async () => {
      await app.database.remove('foo', {})
      await app.database.create('foo', { id: 1, list: [] })
      await app.database.create('foo', { id: 2, list: [23] })
      await app.database.create('foo', { id: 3, list: [233] })
      await app.database.create('foo', { id: 4, list: [233, 332] })
    })
  ...
  export const evaluation = function Evaluation(app: App) {
    before(async () => {
      await app.database.remove('foo', {})
      await app.database.create('foo', { id: 1, value: 8 })
      await app.database.create('foo', { id: 2, value: 7 })
      await app.database.create('foo', { id: 3, value: 9 })
    })
  ...
  namespace Logical {
    export const queryLevel = function LogicalQueryLevel(app: App) {
      before(async () => {
        await app.database.remove('foo', {})
        await app.database.create('foo', { id: 1 })
        await app.database.create('foo', { id: 2 })
        await app.database.create('foo', { id: 3 })
      })

      it('edge cases', async () => {
        await expect(app.database.get('foo', {})).eventually.to.have.length(3)
        await expect(app.database.get('foo', { $and: [] })).eventually.to.have.length(3)
        await expect(app.database.get('foo', { $or: [] })).eventually.to.have.length(0)
        await expect(app.database.get('foo', { $not: {} })).eventually.to.have.length(0)
        await expect(app.database.get('foo', { $not: { $and: [] } })).eventually.to.have.length(0)
        await expect(app.database.get('foo', { $not: { $or: [] } })).eventually.to.have.length(3)
      })
    ...
    export const fieldLevel = function LogicalFieldLevel(app: App) {
      ...
      it('edge cases', async () => {
        await expect(app.database.get('foo', { id: {} })).eventually.to.have.length(3)
        await expect(app.database.get('foo', { id: { $and: [] } })).eventually.to.have.length(3)
        await expect(app.database.get('foo', { id: { $or: [] } })).eventually.to.have.length(0)
        await expect(app.database.get('foo', { id: { $not: {} } })).eventually.to.have.length(0)
        await expect(app.database.get('foo', { id: { $not: { $and: [] } } })).eventually.to.have.length(0)
        await expect(app.database.get('foo', { id: { $not: { $or: [] } } })).eventually.to.have.length(3)
      })
