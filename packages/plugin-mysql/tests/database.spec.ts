...
describe('Mysql Database', () => {
  describe('createFilter', () => {
    ...
    it('compile expr query', () => {
      expect(createFilter('foo', { id: { $gt: 2 } })).equal('`id` > 2')

      expect(createFilter('foo', { id: { $eq: 2 } })).equal('`id` = 2')

      expect(createFilter('foo', { id: { $ne: 2 } })).equal('`id` != 2')
    ...
    it('should verify empty array', () => {
      expect(createFilter('foo', [])).equal('0')

      expect(createFilter('foo', { id: [] })).equal('0')

      expect(createFilter('foo', { id: { $in: [] } })).equal('0')

      expect(createFilter('foo', { id: { $nin: [] } })).equal('1')
    })
