...
type ActionCallback <T extends {}, K extends keyof T> =
  (this: Context, session: Session<'authority'>, target: Observed<Pick<T, K>>, ...args: string[]) => Promise<any>
...
UserAction.add('setAuth', async (session, user, value) => {
  ...
  if (!isInteger(authority) || authority < 0) return session.$send('参数错误。')
  if (authority >= session.$user.authority) return session.$send('权限不足。')
...
UserAction.add('setFlag', async (session, user, ...flags) => {
  ...
  if (!flags.length) return session.$send(`可用的标记有 ${userFlags.join(', ')}。`)
  const notFound = difference(flags, userFlags)
  if (notFound.length) return session.$send(`未找到标记 ${notFound.join(', ')}。`)
...
UserAction.add('setUsage', async (session, user, name, _count) => {
  const count = +_count
  if (!isInteger(count) || count < 0) return session.$send('参数错误。')
...
UserAction.add('setTimer', async (session, user, name, offset) => {
  if (!name || !offset) return session.$send('参数不足。')
  const timestamp = parseTime(offset)
  if (!timestamp) return session.$send('请输入合法的时间。')
...
GroupAction.add('setFlag', async (session, group, ...flags) => {
  ...
  if (!flags.length) return session.$send(`可用的标记有 ${groupFlags.join(', ')}。`)
...
GroupAction.add('setAssignee', async (session, group, _assignee) => {
  ...
  if (!isInteger(assignee) || assignee < 0) return session.$send('参数错误。')
...
export function apply (ctx: Context) {
  ctx.command('admin <action> [...args]', '管理用户', { authority: 4 })
    ...
    .action(async ({ session, options }, name, ...args) => {
      ...
      if ('user' in options && isGroup) return session.$send('不能同时目标为指定用户和群。')
      ...
      if (!name) return session.$send(`当前的可用指令有：${actionList}。`)

      const action = actionMap[paramCase(name)]
      if (!action) return session.$send(`指令未找到。当前的可用指令有：${actionList}。`)
      ...
      if (isGroup) {
        ...
        if (options.thisGroup) {
          ...
          if (!data) return session.$send('未找到指定的群。')
        ...
        if (options.user) {
          ...
          if (!qq) return session.$send('未指定目标。')
          const data = await ctx.database.getUser(qq, -1, fields)
          if (!data) return session.$send('未找到指定的用户。')
