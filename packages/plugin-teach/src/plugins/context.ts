...
export default function apply(ctx: Context, config: Dialogue.Config) {
  ...
  ctx.command('teach')
    ...
    .option('disableGlobal', '-D  在所有环境下禁用问答', { authority: config.switchContextAuthority })
    .option('enable', '-e  在当前环境下启用问答')
    .option('enableGlobal', '-E  在所有环境下启用问答', { authority: config.switchContextAuthority })
    .option('groups', '-g <gids>  设置具体的生效环境', { authority: config.switchContextAuthority, type: 'string', validate: RE_GROUPS })
