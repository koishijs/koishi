...
import { Context, Database, Logger, makeArray, sleep } from 'koishi'
...
class MysqlDatabase extends Database {
  ...
  constructor(public ctx: Context, config?: Config, private transactionClient?: PoolConnection) {
    super(ctx)
    this.config = {
      host: 'localhost',
      port: 3306,
  ...
  async transaction<T>(transactionFun: (_this: MysqlDatabase) => Promise<T>): Promise<T> {
    ...
    const transactionalDatabase = new MysqlDatabase(this.ctx, this.config, transactionClient)
    try {
